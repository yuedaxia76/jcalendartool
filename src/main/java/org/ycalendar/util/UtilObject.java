...
public class UtilObject {
	public static Logger log = Logger.getLogger(UtilObject.class.getName()); 
	...
	private static void setMethodAccessible(final Method method) {
		try {
			...
			log.log(Level.WARNING, "Cannot setAccessible on method. Therefore cannot use jvm access bug workaround.", se);
		}
	}
	 ...
	 * 
	 * <p>
	 * 该方法被 {@link #runMethod(Object object,String methodName,Object [] args,Class[] parameterTypes)}使用.
	 * 
	 * <p>
	...
	private static final Method getMatchingAccessibleMethod(Class<?> clazz, String methodName, Class<?>[] parameterTypes) {
		...
		for (int i = 0, size = methods.length; i < size; i++) {
			if (methods[i].getName().equals(methodName)) {
				...
				log.finest("Found matching name:"+ methods[i]);
				...
				if (methodParamSize == paramSize) {
					...
					for (int n = 0; n < methodParamSize; n++) {
						if (log.isLoggable(Level.FINEST)) {
							log.finest("Param=" + parameterTypes[n].getName());
							log.finest("Method=" + methodsParams[n].getName());
						}
						if (!isAssignmentCompatible(methodsParams[n], parameterTypes[n])) {
							if (log.isLoggable(Level.FINEST)) {
								log.finest(methodsParams[n] + " is not assignable from " + parameterTypes[n]);
							}
							match = false;
							break;
						}
					...
					if (match) {
						...
						if (method != null) {
							if (log.isLoggable(Level.FINEST)) {
								log.finest(method + " accessible version of " + methods[i]);
							}
        ...
        if(log.isLoggable(Level.INFO)) {
        	log.info("No match found. for "+clazz+" name  "+ methodName);	
        }
	...
	private static Method getMethodByName(final Class<?> clazz, final String methodName, final int paramsSize) {
		...
		if (more) {
			log.warning("obj :"+ clazz+"has more than one method  "+  methodName);
			return null;
		}
		return result;
	}
